...
void testNewWord(int nCode)
{
	...
	if (nCode==UTF8_CODE)
	{
    	...
    	strcpy(sInputFile, "/data/lvhongliang/data/ask/all_question_normal.20000");
	}
...
void SplitUTF8()
{
	...
	ICTCLAS_FileProcess("test.txt","testUTF_result.txt");
    ...
    FILE *fp=fopen("test-bigfile.txt","rt");

	if (fp==NULL)
	{
		printf("Error Open test-bigfile.txt\n");
		ICTCLAS_Exit();
		return ;
	}
	char sLine[1024100];
